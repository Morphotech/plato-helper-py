...
from plato_helper_py.api import DEFAULT_TIMEOUT, PlatoError, PlatoUnavailable
from tests.resources import expected_templates, ranger_certificate_schema, ranger_certificate_template, templates_json
...
class TestPlatoHelper(TestCase):
    ...
    def test_get_templates(self, mock_requests):
        ...
        mock_requests.get.assert_called_with(
            f"{PLATO_HOST}/templates/", params={"tags": ["certificate"]}, timeout=DEFAULT_TIMEOUT
        )
    ...
    def test_compose_template(self, mock_requests):
        ...
        mock_requests.post.assert_called_with(
            f"{PLATO_HOST}/templates/{template_id}/compose",
            headers={"accept": "application/pdf"},
            json=self.compose_data,
            params={},
    ...
    def test_compose_template_with_optional_params(self, mock_requests):
        ...
        template_id = "ranger_certificate"
        # all optional params
        file = self.plato_helper.compose(
        ...
        file = self.plato_helper.compose(
            template_id=template_id, compose_data=self.compose_data, mime_type="image/png", page=1
        )
        mock_requests.post.assert_called_with(
            f"{PLATO_HOST}/templates/{template_id}/compose",
            headers={"accept": "image/png"},
    ...
    def test_template_example(self, mock_requests):
        ...
        mock_requests.get.assert_called_with(
            f"{PLATO_HOST}/templates/{template_id}/example",
            headers={"accept": "application/pdf"},
            params={},
            timeout=DEFAULT_TIMEOUT,
        )
    ...
    def test_template_example_with_optional_params(self, mock_requests):
        ...
        template_id = "ranger_certificate"
        # all optional params
        file = self.plato_helper.template_example(
            template_id=template_id, mime_type="image/png", page=1, resize_height=100, resize_width=100
        )
        mock_requests.get.assert_called_with(
            f"{PLATO_HOST}/templates/{template_id}/example",
            headers={"accept": "image/png"},
    ...
    def test_create_template(self, mock_requests):
        ...
        mock_requests.post.assert_called_with(
            f"{PLATO_HOST}/template/create",
            data={"zipfile": file, "template_details": json.dumps(ranger_certificate_schema)},
            timeout=DEFAULT_TIMEOUT,
        )
        ...
        mock_requests.post.assert_called_with(
            f"{PLATO_HOST}/template/create", data={"zipfile": file, "template_details": "{}"}, timeout=DEFAULT_TIMEOUT
        )
        self.assertEqual(file.tell(), 0)
        self.assertEqual(template, ranger_certificate_template)
    ...
    def test_update_template(self, mock_requests):
        ...
        template_id = "ranger_certificate"
        template = self.plato_helper.update_template(
            template_id=template_id, file_stream=file, template_details=ranger_certificate_schema
        )
        mock_requests.put.assert_called_with(
            f"{PLATO_HOST}/template/{template_id}/update",
            data={"zipfile": file, "template_details": json.dumps(ranger_certificate_schema)},
            timeout=DEFAULT_TIMEOUT,
        )
        self.assertEqual(file.tell(), 0)
        ...
        # empty dict
        template = self.plato_helper.update_template(template_id=template_id, file_stream=file, template_details={})
        mock_requests.put.assert_called_with(
            f"{PLATO_HOST}/template/{template_id}/update",
    ...
    def test_update_template_details(self, mock_requests):
        ...
        template_id = "ranger_certificate"
        template = self.plato_helper.update_template_details(
            template_id=template_id, template_details=ranger_certificate_schema
        )
        mock_requests.patch.assert_called_with(
            f"{PLATO_HOST}/template/{template_id}/update_details",
        ...
        # empty dict
        template = self.plato_helper.update_template_details(template_id=template_id, template_details={})
        mock_requests.patch.assert_called_with(
            f"{PLATO_HOST}/template/{template_id}/update_details", json={}, timeout=DEFAULT_TIMEOUT
        )
        self.assertEqual(template, ranger_certificate_template)
